...
async def search_products(query: str, page: int = 1, page_size: int = 10) -> Dict[str, Any]:
    ...
    for p in response.get("products", []):
        ...
        products.append(product_info.model_dump())
...
async def manage_user_restrictions(
    ...
    if action == "get":
        return {"restrictions": [r.model_dump() for r in user.restrictions]}
        ...
        for i, r in enumerate(user.restrictions):
            if r.type == restriction_type and r.value == restriction_value:
                ...
                create_or_update_user(user)
                return {"success": True, "message": "Restriction updated", "restrictions": [r.model_dump() for r in user.restrictions]}
        
        # Add new restriction
        user.restrictions.append(new_restriction)
        create_or_update_user(user)
        return {"success": True, "message": "Restriction added", "restrictions": [r.model_dump() for r in user.restrictions]}
        ...
        create_or_update_user(user)
        return {"success": True, "message": "Restriction removed", "restrictions": [r.model_dump() for r in user.restrictions]}
